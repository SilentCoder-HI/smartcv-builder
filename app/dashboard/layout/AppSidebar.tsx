...
const menu: MenuItem[] = [
  ...
  { name: "My CVs", path: "/my-cvs", icon: <ScrollText size={20} /> },
  { name: "AI Resume Assistant", path: "/ai-assistant", icon: <Sparkles size={20} /> },
  {
    name: "Templates",
    icon: <Layers size={20} />,
    subItems: [
  ...
  { name: "Job Board", path: "/jobs", icon: <BriefcaseBusiness size={20} /> },
  { name: "Job Tracker", path: "/tracker", icon: <ListChecks size={20} /> },
  { name: "Exports", path: "/exports", icon: <Download size={20} /> },
  { name: "Settings", path: "/settings", icon: <Settings size={20} /> },
  { name: "Upgrade to Pro", path: "/upgrade", icon: <Star size={20} className="text-yellow-500" /> },
];
        ...
        {menu.map((item) =>
          item.subItems ? (
                    ...
                    <li key={sub.name}>
                      <button
                        ...
                        className={`block w-full text-left px-2 py-1 rounded-md text-sm hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-0 ${
                          isActive(sub.path) ? "font-semibold text-blue-600" : ""
                        }`}
                      >
                        {sub.name}
                      </button>
            ...
            <button
              ...
              className={`flex items-center gap-3 w-full px-3 py-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-0 ${
                isActive(item.path) ? "bg-gray-100 dark:bg-gray-800 font-semibold text-blue-600" : ""
              }`}
            >
              {item.icon}
